...
export async function loadSettings(): Promise<SiteSettings> {
  ...
  try {
    ...
    else { try { value = mergeSettings(JSON.parse(rows[0].json || "{}")); } catch { value = mergeSettings(null); } }
  } catch (e) { console.error("[db] loadSettings failed (table may be missing)", e); value = mergeSettings(null); }
...
export async function publicSettings(s: SiteSettings) {
  return { ...s, brand: { squareLogoUrl: await signBrandUrl(s.brand.squareLogoUrl), fullLogoUrl: await signBrandUrl(s.brand.fullLogoUrl), faviconUrl: await signBrandUrl(s.brand.faviconUrl) } };
}
