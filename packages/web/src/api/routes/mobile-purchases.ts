...
export function mobilePurchaseRoutes(app: Hono) {
  app.post(
    ...
    async (c) => {
        ...
        return c.json(
          {
            ...
            message:
              "This purchase was already fulfilled. Check your delivery email for the license files.",
          },
          200,
        );
      }
      ...
      try {
        fulfillment = await db.transaction(async (tx) => {
          ...
          if (!beat || !beat.published)
            throw new MobileStoreVerificationError("This beat is unavailable.", 409);
      ...
      try {
        await activateOrderEntitlements(customer!.id, fulfillment.orderId);
        await sendDeliveryEmail(customer!.email, fulfillment.orderId, fulfillment.downloadToken);
        await db
          .update(mobilePurchaseTransactions)
