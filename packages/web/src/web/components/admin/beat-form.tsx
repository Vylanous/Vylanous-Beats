...
export function BeatForm({
  ...
  const set = <K extends keyof BeatInput>(k: K, v: BeatInput[K]) => setForm((f) => ({ ...f, [k]: v }));
  ...
  return (
    <div>
      <button onClick={onCancel} className="flex items-center gap-2 font-body text-sm text-vb-silver/60 hover:text-vb-silver-bright mb-5 transition">
      ...
      <div className="space-y-6 max-w-2xl">
        ...
        {/* Delivery files */}
        <Section title="Delivery files" desc="What buyers download for each license. Upload a zip for multi-file tiers. Only the tiers you sell need files.">
          {TIER_FILES.map((t) => (
            <FileUpload
              key={t.id}
              label={t.label}
        ...
        <Section title="Visibility">
          <Toggle label="Published" desc="Live on the store" checked={form.published} onChange={(v) => set("published", v)} />
          <Toggle label="Featured" desc="Show on homepage spotlight" checked={form.featured} onChange={(v) => set("featured", v)} />
          <Toggle label="Sold exclusively" desc="Mark exclusive license as taken" checked={form.soldExclusive} onChange={(v) => set("soldExclusive", v)} />
        </Section>

        {err && <p className="text-red-400 font-body text-sm">{err}</p>}

        <div className="flex gap-3 pt-2">
          <button onClick={save} disabled={saving} className="flex items-center gap-2 bg-vb-purple hover:bg-vb-purple-bright disabled:opacity-50 text-white font-sub uppercase tracking-wide px-6 py-3 rounded-xl transition">
            {saving ? <Loader2 className="animate-spin" size={18} /> : <Save size={18} />}
            {editing ? "Save changes" : "Publish beat"}
          </button>
          <button onClick={onCancel} className="font-sub uppercase tracking-wide px-6 py-3 rounded-xl text-vb-silver/70 hover:text-vb-silver-bright hover:bg-white/[0.05] transition">
            Cancel
          </button>
        </div>
      </div>
...
function Toggle({ label, desc, checked, onChange }: { label: string; desc: string; checked: boolean; onChange: (v: boolean) => void }) {
  return (
    <button type="button" aria-label={label} onClick={() => onChange(!checked)} className="w-full flex items-center justify-between text-left group">
      ...
      <span className={`relative h-6 w-11 rounded-full transition shrink-0 ${checked ? "bg-vb-purple" : "bg-white/10"}`}>
